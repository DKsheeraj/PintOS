...
thread_preemption (void) {
  if ( !list_empty (&ready_list) ) {
    struct thread *mxReady = list_entry(list_front(&ready_list), struct thread, elem);
    if (intr_context()) { 
      thread_ticks++;
      if (thread_current()->priority < mxReady->priority 
          || (thread_current()->priority == mxReady->priority 
              && thread_ticks >= TIME_SLICE))
        intr_yield_on_return();

    } else if ( thread_current()->priority < mxReady->priority )
      thread_yield();
  }
}
